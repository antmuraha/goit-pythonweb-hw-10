...
async def create_contact_service(
    db: AsyncSession, contact_in: ContactCreate
) -> ContactRead:
    # Avoid duplicate emails
    existing = await get_contact_by_email(db, contact_in.email)
    if existing:
...
async def list_contacts_service(
    ...
    """
    if upcoming:
        return await get_upcoming_birthdays(db, days=7)
...
async def get_upcoming_birthdays_service(db: AsyncSession, days: int = 7):
    return await get_upcoming_birthdays(db, days=days)


async def get_contact_service(db: AsyncSession, contact_id: int):
    return await get_contact_by_id(db, contact_id)


async def update_contact_service(
    db: AsyncSession, contact_id: int, contact_in: ContactUpdate
):
...
async def delete_contact_service(db: AsyncSession, contact_id: int) -> bool:
    contact = await get_contact_by_id(db, contact_id)
    if not contact:
        return False
